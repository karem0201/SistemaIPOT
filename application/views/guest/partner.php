<section id="conatcat-info">
    <div class="container">
        <div class="row">
            <div class="col-sm-8">
                <div class="media contact-info wow fadeInDown" data-wow-duration="1000ms" data-wow-delay="600ms">
                    <div class="pull-left">
                        <i class="fa fa-phone" style="font-size:48px;"></i>
                    </div>
                    <div class="media-body">
                        <h2>¿TIENES ALGUNA INQUIETUD?</h2>
                        <p>Contactanos y te ayudaremos  +01 224 2224 anexo 4070</p>
                    </div>
                </div>
            </div>
        </div>
    </div><!--/.container-->
</section><!--/#conatcat-info-->

<section id="partner">
    <div class="container">
        <div class="center wow fadeInDown">
            <h2>ENCUENTRANOS TAMBIEN</h2>
            <p class="lead">Buscamos mantenerte informado <br> </p>
        </div>

        <div class="partners">
            <ul>
              <li> <a href="#"><img class="img-responsive wow fadeInDown" data-wow-duration="1000ms" data-wow-delay="300ms" src="<?=base_url()?>/plantillas/images/partners/partner1.png"></a></li>
              <li> <a href="#"><img class="img-responsive wow fadeInDown" data-wow-duration="1000ms" data-wow-delay="600ms" src="<?=base_url()?>/plantillas/images/partners/partner2.png"></a></li>
              <li> <a href="#"><img class="img-responsive wow fadeInDown" data-wow-duration="1000ms" data-wow-delay="900ms" src="<?=base_url()?>/plantillas/images/partners/partner3.png"></a></li>
              <li> <a href="#"><img class="img-responsive wow fadeInDown" data-wow-duration="1000ms" data-wow-delay="1200ms" src="<?=base_url()?>/plantillas/images/partners/partner3.png"></a></li>
              <li> <a href="#"><img class="img-responsive wow fadeInDown" data-wow-duration="1000ms" data-wow-delay="1500ms" src="<?=base_url()?>/plantillas/images/partners/partner3.png"></a></li>
            </ul>
        </div>
    </div><!--/.container-->
</section><!--/#partner-->


<section id="bottom">
    <div class="container wow fadeInDown" data-wow-duration="1000ms" data-wow-delay="600ms">
        <div class="row">
            <div class="col-md-3 col-sm-6">
                <div class="widget">
                    <h3>Nosotros</h3>
                    <ul>
                        <li><a href="#">Historia</a></li>
                        <li><a href="#">Convenios</a></li>

                    </ul>
                </div>
            </div><!--/.col-md-3-->

            <div class="col-md-3 col-sm-6">
                <div class="widget">
                    <h3>Especialidades</h3>
                    <ul>
                        <li><a href="#">Staff Médico</a></li>
                        <li><a href="#">Preguntale a tú médico</a></li>
                        <li><a href="#">Testimonios</a></li>
                    </ul>
                </div>
            </div><!--/.col-md-3-->

            <div class="col-md-3 col-sm-6">
                <div class="widget">
                    <h3>Servicios</h3>
                    <ul>
                        <li><a href="#">Tienda de Ortopedia</a></li>
                        <li><a href="#">Podología</a></li>
                    </ul>
                </div>
            </div><!--/.col-md-3-->

            <div class="col-md-3 col-sm-6">
                <div class="widget">
                    <h3>Traumatología al día</h3>
                    <ul>
                        <li><a href="#">Fractura de Calc&aacute;neo</a></li>
                        <li><a href="#">Bienal 2016</a></li>
                        <li><a href="#">Bienal 2015</a></li>
                    </ul>
                </div>
            </div><!--/.col-md-3-->
        </div>
    </div>
</section><!--/#bottom-->
