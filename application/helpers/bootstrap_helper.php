...
if(!function_exists('form_input_select')){
	function form_input_select($name = '', $hidden = false)
	{
		$div = "";
		if(!$hidden){
			$div .= "<select id='". $name ."' name='". $name ."' class='form-control' required=''>";
		}else{
			$div .= "<select id='". $name ."' name='". $name ."' class='form-control' style='display: none;' required=''>";
		}
		return $div;
	}
}
...
{
    function form_submit($message = '', $id = '', $class = '')
    {
    	$class = $class == '' ? 'btn btn-success btn-block' : $class;
    	return "<div class='form-group'><input type='submit' class='". $class ."' id='". $id ."' value='$message'/></div>";
    }
}
